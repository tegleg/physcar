...
#include "MyMeshComponentStats.h"
#include "DynamicMeshBuilder.h"

DECLARE_CYCLE_STAT(TEXT("Cable Sim"), STAT_Cable_SimTime, STATGROUP_MyMeshComponent);
DECLARE_CYCLE_STAT(TEXT("Cable Solve"), STAT_Cable_SolveTime, STATGROUP_MyMeshComponent);
DECLARE_CYCLE_STAT(TEXT("Cable Collision"), STAT_Cable_CollisionTime, STATGROUP_MyMeshComponent);
DECLARE_CYCLE_STAT(TEXT("Cable Integrate"), STAT_Cable_IntegrateTime, STATGROUP_MyMeshComponent);
...
public:
	...
	FCableSceneProxy(UMyMeshComponent* Component)
...
UMyMeshComponent::UMyMeshComponent(const FObjectInitializer& ObjectInitializer)
...
void UMyMeshComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction * ThisTickFunction)
{
	...
	FMeshParticle& StartParticle = Particles[0];
	...
	FMeshParticle& EndParticle = Particles[NumSegments];
	if (bAttachEnd)
	{
...
FBoxSphereBounds UMyMeshComponent::CalcBounds(const FTransform & LocalToWorld) const
{
	...
	{
		const FMeshParticle& Particle = Particles[ParticleIdx];
...
void UMyMeshComponent::QuerySupportedSockets(TArray<FComponentSocketDescription>& OutSockets) const
{
...
bool UMyMeshComponent::HasAnySockets() const
{
	return (Particles.Num() >= 2);
}

bool UMyMeshComponent::DoesSocketExist(FName InSocketName) const
{
	return (InSocketName == CableEndSocketName) || (InSocketName == CableStartSocketName);
}

FTransform UMyMeshComponent::GetSocketTransform(FName InSocketName, ERelativeTransformSpace TransformSpace) const
{
...
void UMyMeshComponent::OnRegister()
{
	Super::OnRegister();
	...
	{
		FMeshParticle& Particle = Particles[ParticleIdx];
...
void UMyMeshComponent::SetAttachEndTo(AActor * Actor, FName ComponentProperty, FName SocketName)
{
...
AActor * UMyMeshComponent::GetAttachedActor() const
{
	return AttachEndTo.OtherActor;
}

USceneComponent * UMyMeshComponent::GetAttachedComponent() const
{
	return AttachEndTo.GetComponent(GetOwner());
}

void UMyMeshComponent::GetCableParticleLocations(TArray<FVector>& Locations) const
{
	Locations.Empty();
	for (const FMeshParticle& Particle : Particles)
	{
		Locations.Add(Particle.Position);
	}
}

void UMyMeshComponent::SolveConstraints()
{
	...
	for (int32 IterationIdx = 0; IterationIdx < SolverIterations; IterationIdx++)
	{
		...
		{
			FMeshParticle& ParticleA = Particles[SegIdx];
			FMeshParticle& ParticleB = Particles[SegIdx + 1];
		...
		if (bEnableStiffness)
		{
			...
			{
				FMeshParticle& ParticleA = Particles[SegIdx];
				FMeshParticle& ParticleB = Particles[SegIdx + 2];
...
void UMyMeshComponent::VerletIntegrate(float InSubstepTime, const FVector & Gravity)
{
	...
	for (int32 ParticleIdx = 0; ParticleIdx<NumParticles; ParticleIdx++)
	{
		...
		if (Particle.bFree)
		{
			...
			//removing this stops it bouncing//
...
/** Solve a single distance constraint between a pair of particles "this bit puts it back to "rest position"*/
void UMyMeshComponent::SolveDistanceConstraint(FMeshParticle& ParticleA, FMeshParticle& ParticleB, float DesiredDistance)
{
...
void UMyMeshComponent::PerformCableCollision()
{
...
void UMyMeshComponent::PerformSubstep(float InSubstepTime, const FVector & Gravity)
{
...
void UMyMeshComponent::GetEndPositions(FVector & OutStartPosition, FVector & OutEndPosition)
{
